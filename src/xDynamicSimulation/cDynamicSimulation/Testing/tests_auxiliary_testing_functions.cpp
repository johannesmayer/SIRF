...
bool test_aux_test_funs::test_pseudospiral_trajectory( void )
{
	...
	try
	{
		...
		size_t const num_arms = 1;
		size_t const pts_per_arm = 10;
		ISMRMRD::MatrixSize mat_size(64,64,64);

		std::vector< std::pair<uint16_t,uint16_t>> traj = aux_test::pseudospiral_trajectory(num_arms, pts_per_arm, mat_size );

		for(int i=0; i<traj.size(); ++i)
		{
			std::cout << "Element " << i << std::endl;
			std::cout << "(" << traj[i].first << "," << traj[i].second  << ")"<< std::endl;
